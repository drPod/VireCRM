...
- **Lovable migration Phase 1 — set new secrets** (deliberately not auto-run; keys are user-scoped + irreversible until re-rotated):
  ...
  - Verify Resend domain `notify.vireonx.space` in the Resend dashboard — SPF + DKIM records must show green before the queue dispatcher can actually deliver email. AI swap unblocks regardless; only email send-time depends on DNS.
