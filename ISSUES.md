...
- **Live `cron.job` on current Supabase project (`coynbufhejaeuifpvmvw`) has ONE row: `drain-workflow-queue`.** Discovered 2026-05-17 evening while researching item-1 fallback options. Migration `20260417054233_*.sql` tries to schedule `send-pending-welcomes` against `https://auto-pilot-sales-ace.lovable.app/hooks/send-pending-welcomes` (dead Lovable host) with a Bearer token whose JWT is signed by `mtcthkzvpfctjanehgdr` — the OLD Supabase project ref. So that schedule never actually landed on this project (or did and was later unscheduled). Effect: welcome emails enqueued by `enqueue_welcome_email` sit forever unless `/hooks/send-pending-welcomes` is poked manually. If the user wants the post-signup welcome to actually go out, write a new migration mirroring `20260517160500_schedule_workflow_drain_cron.sql` to `net.http_post` `https://genesisxsx.darsh-pod.workers.dev/hooks/send-pending-welcomes` every minute with `x-cron-secret`. Same `cron_secret` vault entry already used by `drain-workflow-queue`. Same fix would handle item-1 option (c) for the classify-contact-submissions sweeper. **Not auto-applied — migration = shared state.**
