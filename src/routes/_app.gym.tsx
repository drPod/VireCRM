    ...
    <div className="container mx-auto p-6 space-y-6">
      <header>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          At-risk prediction and goal tracking for your members.
        </p>
      </header>
                ...
                >
                  <div>
                    <div className="text-sm font-medium text-foreground">
                      {leads[r.lead_id]?.name ?? "Unknown"}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {r.visits_last_30} visits last 30d (was {r.visits_prior_30}) · last visit{" "}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Member goals</CardTitle>
        </CardHeader>
        <CardContent>
          {withGoals.length === 0 ? (
            ...
            <div className="space-y-3">
              {withGoals.map((r) => {
                const pct = r.goal_target
                  ? Math.min(100, Math.round(((r.goal_current ?? 0) / r.goal_target) * 100))
                  : 0;
                return (
                  <div key={r.id} className="space-y-1.5">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">
                        {r.goal_current ?? 0} / {r.goal_target} {r.goal_unit ?? ""}
                      </span>
                    </div>
                    <Progress value={pct} />
