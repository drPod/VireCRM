 ...
 *
 * Every minute the scheduler POSTs here with x-cron-secret. The hook:
 *   1. Picks up queued runs whose next_attempt_at is due (or null).
 *   2. Picks up paused runs whose paused_until has elapsed (re-queues them).
 *   3. Calls runOne() per run — that advances the graph as far as it can.
 *
...
export const Route = createFileRoute("/api/public/hooks/run-workflows")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        // Match the sibling hook auth model: shared secret in x-cron-secret.
        // Vercel Cron sends Authorization: Bearer <secret>, so accept both
        // so the hook works whether the scheduler is external or Vercel Cron.
