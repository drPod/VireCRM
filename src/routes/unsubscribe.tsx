...
function UnsubscribePage() {
  ...
  useEffect(() => {
    ...
    fetch(`/email/unsubscribe?token=${encodeURIComponent(t)}`)
      ...
      .catch(() => setState({ kind: "invalid", message: "Unable to verify token." }));
  }, []);
        ...
        {state.kind === "ready" && (
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              You'll stop receiving messages from us. You can resubscribe by contacting support.
            </p>
            <Button variant="command" className="mt-6 w-full" onClick={handleConfirm}>
              Confirm unsubscribe
            </Button>
          </div>
        )}
