...
import { classifySignInError, messageForTenantError } from "../app/lib/auth-errors";
...
describe("classifySignInError", () => {
  ...
  it("maps over_request_rate_limit code to rate-limit copy", () => {
    expect(classifySignInError({ code: "over_request_rate_limit" }).userMessage).toMatch(
  ...
  it("falls back to generic copy for non-Error values", () => {
    expect(classifySignInError("string").userMessage).toBe("Sign-in failed. Please try again.");
    expect(classifySignInError(null).userMessage).toBe("Sign-in failed. Please try again.");
    expect(classifySignInError(undefined).userMessage).toBe("Sign-in failed. Please try again.");
  });

  it("never echoes the original error message to userMessage", () => {
    const secret = {
      code: "invalid_credentials",
      message: "user@example.com not found in tenant-foo",
    };
...
describe("messageForTenantError", () => {
  ...
  it("maps CUSTOMER_PORTAL_NOT_ALLOWED to customer-portal copy", () => {
    expect(messageForTenantError("CUSTOMER_PORTAL_NOT_ALLOWED")).toMatch(/customer account/i);
  });
  ...
  it("returns generic fallback for unknown code", () => {
    expect(messageForTenantError("SOMETHING_ELSE")).toBe("Sign-in failed. Please try again.");
  });
