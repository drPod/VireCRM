...
describe.skipIf(!hasTestDb)("ESIs CRUD", () => {
  ...
  it("POST /api/esis — 400 when serviceAddressId belongs to a different tenant", async () => {
    const fx = await seed();
    const token = await mintJwt({ tenantId: fx.tenantA });
    // FK alone doesn't enforce tenant isolation; query layer verifies ownership.
    const CROSS_TENANT_ESI = "10443720999000001";
    const res = await SELF.fetch(url(HOST_TENANT_A, "/api/esis"), {
      method: "POST",
      headers: {
        authorization: `Bearer ${token}`,
      ...
      body: JSON.stringify({
        serviceAddressId: fx.serviceAddressIdB,
        esiId: CROSS_TENANT_ESI,
      }),
    });
    expect(res.status).toBe(400);
    const body = (await res.json()) as {
      error?: { code?: string; details?: { serviceAddressId?: string } };
    };
    expect(body.error?.code).toBe("VALIDATION");
    expect(body.error?.details?.serviceAddressId).toBe("unknown");
    ...
    expect(bBody.items.find((i) => i.esiId === CROSS_TENANT_ESI)).toBeUndefined();
  });
  ...
  it("PATCH /api/esis/:id — esiId in body is silently stripped (immutable canonical key)", async () => {
    ...
    const ORIGINAL_ESI = "10443720777000001";
    const create = await SELF.fetch(url(HOST_TENANT_A, "/api/esis"), {
      method: "POST",
      headers: {
        authorization: `Bearer ${token}`,
    ...
    // esiId is immutable: UpdateBody is .strict() and omits it, so PATCH 400s.
    const patch = await SELF.fetch(url(HOST_TENANT_A, `/api/esis/${created.id}`), {
      method: "PATCH",
      headers: {
        authorization: `Bearer ${token}`,
    ...
    const after = (await get.json()) as { esiId: string };
    expect(after.esiId).toBe(ORIGINAL_ESI);
  });
